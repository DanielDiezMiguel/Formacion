...
public class ReadPersonaUseCase implements ReadPersonaPort {
    ...
    @Override
    public List<PersonaOutputDTO> getDataOrderByField(HashMap<String, Object> conditions) throws Exception {
        ...
        conditions.forEach((field, value) -> {
            ...
            switch (field) {
                ...
                case "name":
                    predicateList.add(criteriaBuilder.like(personaEntityRoot.get(field), "%" +  (String) value + "%"));
                    break;
...
//        TypedQuery<PersonaEntity> personaEntityTypedQuery = entityManager.createQuery(criteriaQuery);
//        personaEntityTypedQuery.setFirstResult(page - 1).setMaxResults(maxResult).getResultList().forEach(personaEntity -> personaOutputDTOList.add(new PersonaOutputDTO(personaEntity)));
        ...
        entityManager.createQuery(criteriaQuery).getResultList().forEach(personaEntity -> personaOutputDTOList.add(new PersonaOutputDTO(personaEntity)));
        return personaOutputDTOList;
